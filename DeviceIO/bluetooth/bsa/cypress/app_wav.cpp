...
int app_wav_open_file(const char *p_fname, tAPP_WAV_FILE_FORMAT *p_format)
{
    ...
    if (size != sizeof(riff))
    {
        APP_ERROR1("Length read does not match RIFF header (%d != %d)", (int)size, (int)sizeof(riff));
        goto app_wav_open_file_error;
    }
    ...
    do
    {
        ...
        if (size != sizeof(data))
        {
            APP_ERROR1("Length read does not match header (%d != %d)", (int)size, (int)sizeof(data));
            goto app_wav_open_file_error;
        }
        ...
        else if (!memcmp(&data[0], "fact", 4))
        {
            ...
            {
                APP_ERROR1("Length read does not match FACT chunk (%d != %d)", (int)size, (int)sizeof(data));
                goto app_wav_open_file_error;
            }
        }
                  ...
                  !memcmp(&data[0], "cue ", 4))
        {
            ...
            {
                APP_ERROR1("Failed bypassing the LIST chunk (%lu)", tmpU32);
                goto app_wav_open_file_error;
            }
            if (size != sizeof(data))
            {
                APP_ERROR1("Length read does not match LIST chunk (%d != %d)", (int)size, (int)sizeof(data));
                goto app_wav_open_file_error;
            }
        }
