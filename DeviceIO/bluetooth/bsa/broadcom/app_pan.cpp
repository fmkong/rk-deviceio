...
#define APP_PAN_NB_CON_MAX       1//7   /* Maximum number of connections */
...
RK_BT_PAN_EVENT_CALLBACK app_pan_event_cb = NULL;
...
static int app_pan_con_find_by_address(BD_ADDR bd_addr);
...
static void app_pan_send_event(RK_BT_PAN_EVENT event, BD_ADDR bd_addr)
{
    char address[20];
    ...
    app_pan_event_cb(event, address);
}
...
/******************************************************************************
 **
 ** Function         app_pan_con_find_by_address
 **
 ** Description      Find valid con entry by address
 **
 ** Returns          con_index
 **
 ******************************************************************************/
static int app_pan_con_find_by_address(BD_ADDR bd_addr)
{
    ...
    for(con_index = 0; con_index < APP_PAN_NB_CON_MAX; con_index++) {
        if ((app_pan_cb.con[con_index].in_use != FALSE) &&
                (bdcmp(app_pan_cb.con[con_index].bd_addr, bd_addr) == 0))
            break;
    }
...
static void app_pan_rx_open_evt(tBSA_PAN_MSG *p_data)
{
    ...
    if (p_data->open.status == BSA_SUCCESS) {
        ...
        if (con_index != APP_PAN_NB_CON_MAX) {
            ...
            app_pan_send_event(RK_BT_PAN_CONNECT, p_data->open.bd_addr);
        }
    } else {
        APP_ERROR0("Fail : BSA_PAN_OPEN_EVT");
        app_pan_send_event(RK_BT_PAN_CONNECT_FAILED, p_data->open.bd_addr);
    }
...
static void app_pan_rx_close_evt(tBSA_PAN_MSG *p_data)
{
    ...
    if (con_index == APP_PAN_NB_CON_MAX) {
        APP_DEBUG1("not find match con, p_data->close.handle: %d", p_data->close.handle);
        return;
    }

    app_pan_send_event(RK_BT_PAN_DISCONNECT, app_pan_cb.con[con_index].bd_addr);
    app_pan_con_free(con_index);
    app_pan_con_display();
}
...
int app_pan_start(void)
{
    ...
    app_create_thread(app_pan_netif_read_thread, 0, 0, &app_pan_cb.netif_thread);
    return 0;
}
 ...
 **
 ******************************************************************************/
int app_pan_set_role(tBSA_PAN_ROLE role)
{
    tBSA_STATUS status;
 ...
 **
 ******************************************************************************/
int app_pan_open(BD_ADDR bd_addr)
{
 ...
 **
 ******************************************************************************/
int  app_pan_close(BD_ADDR bd_addr)
{
    tBSA_STATUS status;
    ...
    connection = app_pan_con_find_by_address(bd_addr);
    if(connection == APP_PAN_NB_CON_MAX) {
        APP_ERROR0("not find match connection");
        return -1;
    }
