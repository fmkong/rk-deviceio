...
void InfoLed::mainloop() {
    ...
    while(m_isRunning) {
        ...
        case State::WIFI_MODE:
            ...
            setState(State::IDLE);
            break;
        case State::WIFI_MODE_CONNECTING:
            fprintf(stderr,"-----mode WIFI_MODE_CONNECTING-------\n");
            m_ledState = State::WIFI_MODE_CONNECTING;
            bitmap = 0x1f;
            leds_multi_set_scroll(bitmap,LED_MULTI_PURE_COLOR_NON_GREEN,LED_MULTI_PURE_COLOR_BLACK, 3, 50);
            setState(State::IDLE);
            break;
        case State::BT_MODE:
...
void InfoLed::led_open(int mode, int val) {
    ...
    case MODE_WIFI_CONNECTING:
        //led_all_on(COLOR_GREEN);
        setState(State::WIFI_MODE_CONNECTING);
        mStateChange.notify_one();
        break;
    case MODE_BLUETOOTH:
...
static int pcba_cx20921_upgrade()
{
    int ret = -1;
    char buf[256] = {0};
// because in kernel 4.4, we change the gpio driver, we use 1000+gpio_number to identify a gpio in sys/class/gpio/
#if  LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0)
    char name[256] = "/data/cx20921/i2c_flash -c -d /dev/i2c-2 -g 1028 -f /data/cx20921/evk-nebula-generic.sfs /data/cx20921/iflash.bin";
#else
